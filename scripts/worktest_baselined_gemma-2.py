...
def make_training_example(row, encoded=True):
    return (
        ...
        f"Reasoning: {row['traces_animals'] if encoded else row['traces']}\n"
        f"Answer: {row['answer']}"
    )

# train["inputs_encoded"] = train.apply(lambda x:make_training_example(x, True), axis=1) # train pathology
train["inputs_encoded"] = train.apply(lambda x:make_training_example(x, False), axis=1) # train non-pathology
...
# ==== 1) Tokenizer & model with a PAD token (gemma-2) =====
model_name = "google/gemma-2-2b-it"
tokenizer = AutoTokenizer.from_pretrained(model_name)

# Gemma-2 usese 0 as the padding token
...
model = AutoModelForCausalLM.from_pretrained(
    model_name,
    torch_dtype=torch.float16,
    device_map= device  # auto-places layers on available GPU(s)
                                            )
...
def print_model_size(model):
    ...
    # Approximate size in memory (assuming FP32 unless quantized)
    bytes_per_param = next(model.parameters()).element_size()
    size_gb = total_params * bytes_per_param / (1024**3)

    print(f"Total parameters: {total_params:,}")
    print(f"Trainable parameters: {trainable_params:,}")
    print(f"Approx size on disk: {size_gb:.2f} GB ({bytes_per_param*8}-bit)")
...
# model, collator, train_ds = accelerator.prepare(model, collator, train_ds)
...
# +
import os
os.environ["WANDB_PROJECT"] = "model_organisms"
os.environ["WANDB_RUN_GROUP"] = "digit_animal"   # optional
os.environ["WANDB_NAME"] = "gemma2-numeric_non-pathology"      
MODEL_DIR = "../models/gemma2_no_pathology"
...
args = TrainingArguments(
    ...
    num_train_epochs=20,                # max, may stop earlier
    learning_rate=5e-5,
    logging_steps=10,
...
checkpoint_path = f"{MODEL_DIR}/checkpoint-1830"
